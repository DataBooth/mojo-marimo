...
x_small, y_small, inside_small, pi_small, error_small = monte_carlo_mojo_ext.generate_samples(10_000)
...
x_large, y_large, inside_large, pi_large, error_large = monte_carlo_mojo_ext.generate_samples(1_000_000)
...
fig = make_subplots(
    rows=1, cols=2,
    subplot_titles=('π Estimate vs Sample Size', 'Absolute Error vs Sample Size')
)

# Estimates
fig.add_trace(
    go.Scatter(x=sample_sizes, y=estimates, mode='lines+markers', name='Estimate'),
    row=1, col=1
)
...
# Errors
fig.add_trace(
    go.Scatter(x=sample_sizes, y=errors, mode='lines+markers', name='Error', 
               line=dict(color='red')),
    row=1, col=2
)
